...
var ORDER = { [HEALTH.HEALTHY]: 0, [HEALTH.NEEDS_REVIEW]: 1, [HEALTH.BROKEN]: 2 };
var LABEL = {
...
function renderHealthMarker(el, { state, reason }) {
...
async function guardWrite(label, fn) {
  try {
    return await fn();
  } catch (e) {
    new import_obsidian.Notice(`${label} failed: ${e.message || e}`, 8e3);
    throw e;
  }
}
...
function timeAgo(ts) {
  const tsMs = typeof ts === "number" ? ts : ts instanceof Date ? ts.getTime() : ts ? Date.parse(ts) : NaN;
  if (!tsMs || Number.isNaN(tsMs)) return "\u2014";
  const min = Math.max(0, Math.floor((Date.now() - tsMs) / 6e4));
  if (min < 1) return "just now";
  if (min < 60) return `${min} min ago`;
  if (min < 48 * 60) return `${Math.floor(min / 60)} h ago`;
  return `${Math.floor(min / 1440)} d ago`;
}
function projectIndex(app, root) {
  const out = [];
...
function addSettingsGear(parentEl, app, pluginId, label = "Plugin settings") {
...
function buildShell(containerEl, { title, stage, settings } = {}) {
  containerEl.empty();
...
function setShellHealth(healthEl, health) {
  healthEl.empty();
...
var ShellPlugin = class extends import_obsidian.Plugin {
  scaffold() {
  ...
  async onload() {
    ...
    if (c.protocol) {
      ...
      const argOf = typeof c.protocol === "object" && c.protocol.arg ? c.protocol.arg : () => void 0;
  ...
  async activateView(arg) {
    ...
    if (!leaf) {
      if (arg !== void 0 && arg !== null) this.stashPending(arg);
      leaf = workspace.getLeaf(true);
      await leaf.setViewState({ type: viewType, active: true });
    } else {
      if (arg !== void 0 && arg !== null) this.applyViewArg(leaf.view, arg);
...
var ShellItemView = class extends import_obsidian.ItemView {
  ...
  async onOpen() {
    this.regions = buildShell(this.contentEl, {
      title: this._title,
      stage: this._stage,
      settings: this._pluginId ? { app: this.app, pluginId: this._pluginId, label: `${this._title || "Plugin"} settings` } : void 0
    });
...
// src/main.js
var ordinalCompare = (a, b) => a < b ? -1 : a > b ? 1 : 0;
var VIEW_TYPE = "dev-board-view";
var REVIEW_VIEW_TYPE = "dev-board-review-view";
var BIG_VIEW_TYPE = "dev-board-big-view";
var PLUGIN_ID = "dev-board";
var DOCS_URL = "https://github.com/cobbenterprises/obsidian-dev-board";
var NOTE_ENTRY_RE = /^- \*\*(.+?)\*\*\s+—\s+(.*?)\s*`(dd-[a-z0-9-]+)`\s*$/;
var DATE_HEAD_RE = /^## (\d{4}-\d{2}-\d{2})\s*$/;
...
var DEFAULT_SETTINGS = {
  ...
  // [sifi] dd-20260821-2fqz: empty columns collapse to slim stubs on desktop
  autoLiveDays: 7,
  // [sifi] owner rule 2026-09-05: a card with no Doesn't for N days advances to Lived; 0 = off
  projectRoot: "",
  ownerLabel: "Me",
  agentLabel: "Agent",
  subsystems: ""
...
var BIG_IN_FLIGHT_COLUMNS = ["next", "finalize", "building"];
var BIG_SHIPPED_COLUMNS = ["shipped", "reviewed", "lived"];
var NEXT_STEP_HEAD_RE = /^#{2,4}\s+Your (?:next step|sign-?off)\b.*$/im;
...
var LINK_TOKEN_RE = /\[\[([^\]|#]+)(?:#[^\]|]+)?(?:\|([^\]]+))?\]\]|\b(?:obsidian|https?):\/\/[^\s)>\]]+|(?:^|[\s`(])((?:[0-9A-Za-z][^\n`]*?\/)?[^\n`/]+\.md)(?=$|[\s`),.;])/g;
...
function decorateNextSteps(containerEl) {
  ...
  for (const h of headings) {
    ...
    while (node) {
      ...
      if (isHeading && Number(tag[1]) <= level || innerHeading && Number(innerHeading.tagName[1]) <= level) break;
...
var DevBoardPlugin = class extends ShellPlugin {
  scaffold() {
    return {
      viewType: VIEW_TYPE,
  ...
  async initState() {
    ...
    this.bigError = null;
    this._saving = false;
    this._nextStepCache = /* @__PURE__ */ new Map();
  }
  ...
  async afterLoad() {
    ...
    this.registerObsidianProtocolHandler(
      "dev-board-review",
      (params) => this.activateReviewView(
        params.cards ? String(params.cards).split(",").filter(Boolean) : void 0
      )
    );
    this.addCommand({
      id: "open-dev-board-triage",
      name: "Open Dev Board Triage",
      callback: () => this.activateReviewView(void 0, "triage")
    });
    this.registerObsidianProtocolHandler(
      "dev-board-triage",
      () => this.activateReviewView(void 0, "triage")
    );
    ...
    this.registerEvent(
      this.app.vault.on("modify", (f) => {
        ...
        if (f.path === this.settings.boardPath || f.path === this.settings.notesPath || f.path === this.settings.bigBoardPath || f.path.startsWith(this.settings.debriefsPath + "/")) {
  ...
  async loadBoard() {
    try {
      ...
      this._recordSnapshot = new Map(
        [...this.board.cards || [], ...this.board.archive || []].map((card) => [card.id, JSON.stringify(card)])
      );
      this.lastError = null;
      await this.autoLiveSweep();
    } catch (e) {
      this.board = null;
  ...
  mergeCardRecord(existing, incoming, event) {
    ...
    const wins = !existing || ordinalCompare(clock.join("\0"), currentClock.join("\0")) >= 0;
    if (wins) {
    ...
    result.history = [...existing?.history || [], ...incoming?.history || []].sort((a, b) => ordinalCompare(`${a.ts || ""}\0${a.action || ""}\0${a.by || ""}`, `${b.ts || ""}\0${b.action || ""}\0${b.by || ""}`)).filter((row) => {
      const key = `${row.ts || ""}\0${row.action || ""}\0${row.by || ""}`;
  ...
  async reduceBoardEvents() {
    ...
    events.sort((a, b) => ordinalCompare(this.eventClock(a).join("\0"), this.eventClock(b).join("\0")));
    let applied = 0;
    const openRecords = /* @__PURE__ */ new Map();
    for (const event of events) {
      ...
      if (!record) {
        ...
        try {
          record = { ...record, ...JSON.parse(await adapter.read(recordPath)) };
        } catch (e) {
        }
      }
      ...
      if (ordinalCompare(incomingClock.join("\0"), (record.clock || ["", ""]).join("\0")) >= 0) {
        record.archived = event.operation === "archive";
        record.clock = incomingClock;
      }
      record.applied_events = [.../* @__PURE__ */ new Set([...record.applied_events || [], event.event_id])].sort();
      record._dirty = true;
      applied++;
    }
    ...
    try {
      ...
      for (const path of (listing.files || []).filter((item) => /\/dd-[^/]+\.json$/.test(item)).sort()) {
        try {
          records.push(JSON.parse(await adapter.read(path)));
        } catch (e) {
        }
      }
    } catch (e) {
    ...
    const sorter = (a, b) => ordinalCompare(`${b.updated || ""}\0${b.id}`, `${a.updated || ""}\0${a.id}`);
    active.sort(sorter);
    archive.sort(sorter);
    const nextBoard = JSON.stringify({ ...shell, cards: active, archive }, null, 2) + "\n";
    let currentBoard = null;
    try {
      currentBoard = await adapter.read(this.settings.boardPath);
    } catch (e) {
    }
  ...
  async emitBoardEvents() {
    const root = this.boardDir();
    const writer = `obsidian-${import_obsidian2.Platform.isMobile ? "mobile" : "desktop"}`;
    const eventDir = `${root}/Events/${writer}`;
    await this.ensureRecordDir(eventDir);
    const snapshot = this._recordSnapshot || /* @__PURE__ */ new Map();
  ...
  newCardId() {
    ...
    const ymd = date.getFullYear() * 1e4 + (date.getMonth() + 1) * 100 + date.getDate();
    const taken = new Set(
...
${body}`;
  ...
  async reverseToBuild(cardId, by = "owner") {
    ...
    (live.history = live.history || []).push({
      ts,
      action: `reversal \u2014 reverted ${live.column} \u2192 building`,
      by
    });
  ...
  async appendImagesToCardFile(path, imagePaths) {
    ...
    await guardWrite(
      "Adding screenshots to the card note",
...
${embeds}${text.slice(insertAt)}`;
  ...
  boardHealth() {
    ...
    checks.push({
      ...
      reason: `${cards.length} cards \xB7 ${rec} recommended`
    });
    return computeHealth(checks);
  }
  subsystemValues() {
    const vals = new Set(
      String(this.settings.subsystems || "").split(",").map((s) => s.trim()).filter(Boolean)
    );
  ...
  openInDefaultApp(vaultRelPath) {
    if (import_obsidian2.Platform.isMobile) {
      new import_obsidian2.Notice("Open this file from a desktop client: " + vaultRelPath);
      return;
    }
    this.app.openWithDefaultApp(vaultRelPath);
  }
};
var DevBoardView = class extends ShellItemView {
  ...
  async refresh(firstRender = false) {
    ...
    if (p.bootstrapWarning) c.createDiv({ cls: "sdd-bootstrap", text: `Dev Board: ${p.bootstrapWarning}` });
    this.renderBoard(c);
    ...
    setStatus(
      r.statusEl,
      `Updated ${(/* @__PURE__ */ new Date()).toLocaleTimeString()} \xB7 record: ${p.settings.boardPath} (agents write it directly)`,
      "info"
    );
  }
  renderActions(el) {
    ...
    const doAdd = async () => {
      ...
      input.value = "";
      new import_obsidian2.Notice("Card added");
      this.refresh();
    };
  ...
  applyBoardFilter() {
    ...
    root.querySelectorAll(".sdd-card").forEach((el) => {
      ...
      const initiativeMiss = initiativeFilter === "__none__" ? !!el.dataset.initiative : initiativeFilter ? el.dataset.initiative !== initiativeFilter : false;
  ...
  renderCard(parent, card) {
    ...
    el.dataset.search = [card.title, card.summary, card.details, card.id, card.subsystem, initiative?.title].filter(Boolean).join(" ").toLowerCase();
  ...
  showCardMenu(evt, card) {
    ...
    for (const col of p.columns()) {
      ...
      menu.addItem(
        (i) => i.setTitle(`Move to ${col.label}`).setIcon("arrow-right").onClick(async () => {
    ...
    menu.addSeparator();
    menu.addItem(
      (i) => i.setTitle("Edit\u2026").setIcon("pencil").onClick(() => this.openCardModal(card))
    );
    menu.addItem(
      (i) => i.setTitle(card.file ? "Open card note" : "Create card note").setIcon("file-pen").onClick(async () => {
    ...
    if (card.project) {
      menu.addItem(
        (i) => i.setTitle("Open project note").setIcon("file-text").onClick(
          () => p.app.workspace.openLinkText(card.project, "", false)
        )
      );
    }
    menu.addItem(
      (i) => i.setTitle("Archive").setIcon("archive").onClick(async () => {
  ...
  renderDebriefs(parent) {
    ...
    for (const d of p.debriefs.slice(0, 10)) {
      ...
      row.createSpan({
        ...
        text: d.cards.length ? toReview ? `${toReview} of ${d.cards.length} to review` : `Reviewed \xB7 ${d.cards.length} cards` : "No cards listed"
      });
    ...
    for (const entry of entries) {
      ...
      promote.addEventListener("click", (e) => {
        ...
        for (const col of p.columns()) {
          ...
          menu.addItem(
            (i) => i.setTitle(`\u2192 ${col.label}`).onClick(async () => {
              await p.promoteEntry(entry, col.id);
              new import_obsidian2.Notice(`${entry.title} \u2192 ${col.label}`);
              this.refresh();
            })
          );
        }
  ...
  renderMaintenance() {
    ...
    m.empty();
    const cards = p.board?.cards || [];
    const counts = p.columns().map((c) => `${c.label}: ${cards.filter((x) => x.column === c.id).length}`).join(" \xB7 ");
    m.createDiv({
      cls: "sdd-stats",
      text: `${counts} \xB7 Archived: ${(p.board?.archive || []).length}. The board JSON is the single source of truth; external writers (your AI agents or scripts) append event files next to it, so cards appear here without a reload.`
    });
...
var ReviewView = class extends ShellItemView {
  constructor(leaf, plugin) {
    super(leaf, {
  ...
  async render() {
    if (this.containerEl && this.containerEl.classList) this.containerEl.classList.add("sdd-review-fit");
    await this.refresh(true);
  }
  ...
  queue() {
    ...
    return shipped.filter((c) => order.has(c.id)).sort((a, b) => order.get(a.id) - order.get(b.id));
  }
  ...
  async refresh(firstRender = false) {
    ...
    bar.createSpan({
      ...
      text: q.length ? `${this.index + 1} of ${q.length} to ${this.mode === "triage" ? "triage" : this.mode === "shipped" ? "review" : "work"}` : "Queue clear"
    });
    ...
    if (this.queueIds) {
      const scope = bar.createEl("button", {
        ...
        text: "Debrief scope \xB7 show all shipped",
    ...
    if (this._lastRoute) {
      ...
      undo.addEventListener("click", async () => {
        ...
        if (live) {
          ...
          (live.history = live.history || []).push({
            ts,
            action: `undo \u2014 moved ${live.column} \u2192 ${move.from}`,
            by: "owner"
          });
          live.column = move.from;
          live.updated = ts;
          await p.saveBoard();
          new import_obsidian2.Notice(`Undone \u2014 back in ${p.columnLabel(move.from)}: ${move.title}`);
        }
        this.refresh();
      });
    }
    ...
    if (!q.length) {
      const done = c.createDiv({ cls: "sdd-review__done" });
        ...
        text: this.mode === "triage" ? "Nothing is waiting in Recommended or Ideas." : this.queueIds ? "Every card in this debrief has been routed." : `Nothing is waiting in ${p.columnLabel(this.mode)}. Pick another queue above.`
      });
    ...
    setStatus(
      r.statusEl,
      `${card.id} \xB7 ${p.columnLabel(card.column).toLowerCase()} ${timeAgo(new Date(card.updated || card.created).getTime())}`,
      "info"
    );
  }
  async renderCard(parent, card) {
    ...
    idChip.addEventListener("click", async () => {
      try {
        ...
        new import_obsidian2.Notice("Copied: " + card.id);
      } catch (e) {
        new import_obsidian2.Notice("Copy failed \u2014 select the title text instead");
      }
    });
    chips.createSpan({
      cls: ["sdd-chip", card.source === "agent" ? "sdd-chip--agent" : "sdd-chip--owner"],
    ...
    const ORIGIN_LABELS = {
      chat: "chat close-out",
      board: "typed on the board",
      briefing: "a briefing",
      import: "imported",
      "daily-log": "your daily log",
      review: "captured mid-review",
      runner: "a runner"
    };
    ...
    if (card.source === "owner") {
      ...
      originBox.createDiv({
        cls: "sdd-review__origin-head",
        text: `\u{1F4A1} ${p.settings.agentLabel} recommended this (${ORIGIN_LABELS[card.origin] || card.origin || "recommendation"}, ${born}) \u2014 why:`
      });
...
${embeds}` : embeds,
      ...
      new import_obsidian2.Notice(`${saved.length} screenshot${saved.length === 1 ? "" : "s"} attached as feedback`);
      this.refresh();
    });
    ...
    fbSend.addEventListener("click", async () => {
      ...
      if (!text) {
        new import_obsidian2.Notice("Write the feedback first.");
        this._fbIn.focus();
        return;
      }
      await p.recordCardFeedback(card, text);
      this._drafts.delete(card.id);
      new import_obsidian2.Notice("Feedback recorded on the card note.");
      this.refresh();
    });
    ...
    if (noteBody) {
      ...
      await import_obsidian2.MarkdownRenderer.render(p.app, body, noteEl, card.file, this);
    ...
    if (col === "reviewed" || col === "lived") {
      ...
      rev.addEventListener("click", async () => {
        ...
        try {
          ...
          new import_obsidian2.Notice(`REVERSED to ${p.columnLabel("building")}: ${card.title}`);
          await this.refresh();
        } finally {
          this._routing = false;
        }
    ...
    rework.addEventListener("click", async () => {
      if (!this._fbIn.value.trim()) {
        new import_obsidian2.Notice("Add feedback first so your agent knows what to fix.");
        this._fbIn.focus();
        return;
      }
  ...
  async route(card, toColumn, noticeText) {
    ...
    try {
      ...
      new import_obsidian2.Notice(noticeText);
      await this.refresh();
    } finally {
      this._routing = false;
    }
...
var CardModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    meta.setText(
      isNew ? `${p.settings.ownerLabel} \xB7 board \xB7 card will be created when saved` : `${card.source === "agent" ? p.settings.agentLabel : p.settings.ownerLabel} \xB7 ${card.origin || "board"} \xB7 created ${String(card.created || "").slice(0, 10)} \xB7 ${card.id}`
    );
    ...
    const detIn = contentEl.createEl("textarea", {
      ...
      attr: {
        placeholder: "Implementation detail \u2014 enough context to build this without the chat it came from: what/why, files & paths, acceptance criteria, links."
      }
    });
    ...
    const renderNoteSection = async () => {
      ...
      if (card.file && p.app.vault.getAbstractFileByPath(card.file)) {
        ...
        const showPreview = async () => {
          preview.empty();
          await import_obsidian2.MarkdownRenderer.render(p.app, this.noteBodyIn.value, preview, card.file, p);
    ...
    const renderImages = () => {
      ...
      for (const path of this.keptImages) {
        ...
        thumb(
          ...
          path.split("/").pop(),
          () => {
            this.keptImages = this.keptImages.filter((x) => x !== path);
            renderImages();
          },
    ...
    p.loadBigBoard().then(() => {
      ...
      for (const initiative of initiatives) {
        const opt = initiativeSel.createEl("option", { text: `${initiative.title} \xB7 ${laneLabel(initiative.lane)}` });
...
${card.details || ""}`;
    ...
    const resolvedNotes = [...notePaths].map((path) => ({ path, file: p.app.metadataCache.getFirstLinkpathDest(path.replace(/\.md$/, ""), "") })).filter(({ file }) => file);
    ...
    if (resolvedNotes.length || linkedUrls.length) {
      ...
      for (const url of linkedUrls) {
        const label = url.startsWith("obsidian://") ? url.replace(/^obsidian:\/\//, "").split("?")[0] : url.replace(/^https?:\/\//, "").split("/")[0];
        const button = notes.createEl("button", {
    ...
    save.addEventListener("click", async () => {
      ...
      if (!title) {
        new import_obsidian2.Notice("Title required");
        return;
      }
      const fields = {
        title,
      ...
      for (let i = 0; i < this.stagedImages.length; i++) {
        try {
          ...
          new import_obsidian2.Notice(`Saving screenshot failed: ${error.message || error}`);
          save.disabled = false;
          return;
        }
      }
      ...
      if (this.initiativeSel && !this.initiativeSel.disabled && this.initiativeSel.value !== this.initiativeOriginal) {
        try {
          ...
          new import_obsidian2.Notice(`Big Board link failed: ${error.message || error}`);
        }
      }
      if (card.file && p.app.vault.getAbstractFileByPath(card.file)) {
        try {
          ...
          new import_obsidian2.Notice(`Card note update failed: ${error.message || error}`);
        }
      }
      if (this.feedbackIn && this.feedbackIn.value.trim()) {
        try {
          await p.recordCardFeedback(p.findCard(cardId) || card, this.feedbackIn.value);
          new import_obsidian2.Notice("Feedback recorded");
        } catch (error) {
          new import_obsidian2.Notice(`Feedback save failed: ${error.message || error}`);
        }
      }
      this.close();
      this.onDone();
...
var DevBoardSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    const bind = (name, desc, key, { allowEmpty = false } = {}) => new import_obsidian2.Setting(containerEl).setName(name).setDesc(desc).addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Auto-advance to Lived after N days").setDesc("A card in Testing, Shipped, or Reviewed with no feedback, verdict, or edit for this many days moves to Lived on its own. 0 turns it off.").addText(
      (t) => t.setValue(String(this.plugin.settings.autoLiveDays)).onChange(async (v) => {
        const n = parseInt(v, 10);
        this.plugin.settings.autoLiveDays = Number.isFinite(n) && n >= 0 ? n : DEFAULT_SETTINGS.autoLiveDays;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Collapse empty columns").setDesc("On desktop, an empty column shrinks to a slim stub; drag over it or click it to open it.").addToggle(
      (t) => t.setValue(this.plugin.settings.collapseEmptyColumns !== false).onChange(async (v) => {
...
var InitiativeModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    let titleIn;
    new import_obsidian2.Setting(contentEl).setName("Initiative").addText((t) => {
      titleIn = t;
      t.setValue(value.title).onChange((v) => value.title = v);
      t.inputEl.style.width = "100%";
    });
    new import_obsidian2.Setting(contentEl).setName("One-line why").addText((t) => {
      t.setValue(value.why).onChange((v) => value.why = v);
      t.inputEl.style.width = "100%";
    });
    new import_obsidian2.Setting(contentEl).setName("Lane").addDropdown((d) => {
      for (const lane of p.bigLanes()) d.addOption(lane.id, lane.label);
      d.setValue(value.lane).onChange((v) => value.lane = v);
    });
    new import_obsidian2.Setting(contentEl).setName("Column").addDropdown((d) => {
      for (const col of p.bigColumns()) d.addOption(col.id, col.label);
      d.setValue(value.column).onChange((v) => value.column = v);
    });
    new import_obsidian2.Setting(contentEl).setName("Linked dd- cards").setDesc("Comma-separated dd- ids; drives the live in-flight/shipped chip.").addText((t) => {
      t.setValue(value.dd).onChange((v) => value.dd = v);
      t.inputEl.style.width = "100%";
    });
    new import_obsidian2.Setting(contentEl).addButton(
      (b) => b.setButtonText(this.initiative ? "Save" : "Add to board").setCta().onClick(async () => {
        if (!value.title.trim()) {
...
var BigBoardView = class extends ShellItemView {
  constructor(leaf, plugin) {
    super(leaf, {
  ...
  async refresh() {
    ...
    setShellHealth(r.healthEl, computeHealth([
      p.bigError ? { state: HEALTH.BROKEN, reason: p.bigError } : { state: HEALTH.HEALTHY, reason: "big_board.json readable" }
    ]));
    if (!p.bigBoard) {
    ...
    setStatus(
      r.statusEl,
      `${(p.bigBoard.initiatives || []).length} initiative(s) on the board \xB7 ${(p.bigBoard.recommendations || []).length} recommendation(s) below`,
      "info"
    );
  }
  renderGrid(parent) {
  ...
  renderInitiative(parent, initiative) {
    ...
    more.addEventListener("click", (e) => {
      ...
      for (const lane of p.bigLanes()) {
        ...
        menu.addItem(
          (i) => i.setTitle(`Lane: ${lane.label}`).setIcon("rows-3").onClick(async () => {
      ...
      menu.addSeparator();
      menu.addItem(
        (i) => i.setTitle("Remove from board").setIcon("archive").onClick(async () => {
  ...
  renderRecommendations(parent) {
    ...
    for (const rec of recs) {
      ...
      main.createSpan({
        ...
        text: `${p.bigLanes().find((l) => l.id === rec.lane)?.label || rec.lane} \xB7 ${p.bigColumns().find((c) => c.id === rec.column)?.label || rec.column}`
      });
